...
class MatMulOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        ...
        if ((node.matmul_attr_trans_A is True) and (node.matmul_attr_trans_B is False)):
        	assert input_shapes[0][0] == input_shapes[1][0]
        	return (input_shapes[0][1], input_shapes[1][1])
        if ((node.matmul_attr_trans_A is False) and (node.matmul_attr_trans_B is True)):
        	assert input_shapes[0][1] == input_shapes[1][1]
        	return (input_shapes[0][0], input_shapes[1][0])
        if ((node.matmul_attr_trans_A is True) and (node.matmul_attr_trans_B is True)):
...
class Executor(object):
    ...
    def memory_plan(self, feed_shapes):
        ...
        infer_shape(feed_shapes)
        for node in self.topo_oreder:
        	self.node_to_arr_map[node] = ndarray(self.node_to_shape_map[node], ctx=self.ctx)
